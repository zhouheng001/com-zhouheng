...
(function (w, $$) {
     ...
     * @return {boolean} true 是货币 | false 非货币
     */
    validate.money_no_zero = function (value) {
        if ((value + '') === "0.0") {
            return false;
        }
    ...
    validate.mobile = function (value) {
        ...
        return /^[1-9]([0-9]{7,11})$/.test(value);
    ...
    validate.email = function (value) {
        return /^[A-Za-z0-9\u4e00-\u9fa5\.\-_]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/.test(value);
    };
...
(function (w, $$) {
     ...
     * @returns {string}
     */
    money.displayFen = function (fen, scale, symbol) {
    ...
    $$.init = function (options) {
        ...
        var settings = {
            data: {
                ...
                dateRangePickerOptions1: {
                    shortcuts: [{
                        text: '最近一周',
                        onClick: function (picker) {
                            var end = new Date();
                            var start = new Date();
                            start = new Date(start - 3600 * 1000 * 24 * 6);
                        ...
                        onClick: function (picker) {
                            var end = new Date();
                            var start = new Date();
                            start = new Date(start - 3600 * 1000 * 24 * 29);
                            picker.$emit('pick', [start, end]);
                        }
                    }, {
                        text: '最近三个月',
                        onClick: function (picker) {
                            var end = new Date();
                            var start = new Date();
                            start = new Date(start - 3600 * 1000 * 24 * 89);
                            picker.$emit('pick', [start, end]);
                        }
                    }]
                },
        ...
        w["$vue" + n] = new Vue({
            ...
            created: function () {
                ...
                if ($UF.isFunction(_uu.global.vue_options.created)) {
                    _uu.global.vue_options.created.call(this);
                }
            },
            mounted: function () {
                if (options.loading) {
                ...
                if ($UF.isFunction(_uu.global.vue_options.mounted)) {
                    _uu.global.vue_options.mounted.call(this);
                }
            },
            data: function () {
                return $UF.extend({}, settings.data);
            },
...
(function (w, v, $$) {
    ...
    var error = function (response, loading, after, context) {
        var msg = "";
        if (!response.ok) {
            msg = "网络异常，错误码:" + response.status + "";
        } else {
            msg = "发生错误";
        }
        ...
        if ($UF.isFunction(after)) {
            context ? after.call(context) : after();
        }
        if (loading) {
            $$.loading("hide");
        }
     ...
     * @param options 其它参数
     *              sync        {boolean}    true 同步请求 | false 异步请求
     *              context     {object}     作用于对象，回调函数执行的作用域
    ...
    http.get = function (url, data, success, options) {
        ...
        if ($UF.isFunction(options.before)) {
            options.context ? options.before.call(options.context) : options.before();
        }
        var o = {};
        var globalError = $UF.isBoolean(options.globalError) ? options.globalError : true;
        o.params = data;
        ...
        if (sync) {
            ...
            options.context ? success.call(options.context, json) : success(json);
        } else {
            v.http.get($UC.ctxPath + url, o).then(function (response) {
                if (globalError && response.data.code && response.data.code !== 0) {
                    v.prototype.$message.error('' + response.body.msg + '');
                } else {
                    options.context ? success.call(options.context, response) : success(response);
                }

                if ($UF.isFunction(options.after)) {
                    options.context ? options.after.call(options.context) : options.after();
                }
                if (loading) {
                    $$.loading("hide");
                }
            }, function (response) {
                error(response, loading, options.after, options.context);
            });
        }
    };
    ...
    http.post = function (url, data, success, options) {
        ...
        v.http.post($UC.ctxPath + url, data, o).then(function (response) {
            if (globalError && response.body.code && response.body.code !== 0) {
                v.prototype.$message.error({
     ...
     * @param options
     */
    http.posthand = function (url, data, success, options) {
        options = options || {};
        if ($UF.isFunction(options.before)) {
            options.context ? options.before.call(options.context) : options.before();
        }
...
Vue.use(
    ...
    (function ($) {
        ...
        var _add_valid_event = function (fi, f) {
            try {
                if (fi.fitype == "select") {
        ...
        var _validate_item = function (item) {
            ...
            var $el = $(item.el), $fi = _get_input(item.el), rules = item.rules, data = item.data, index = item.index, showMsg = item.showMsg;
            ...
            $el.next(".ui-item__error_container").remove();
            ...
            if (Object.prototype.toString.call(rules) !== "[object Array]") {
                result = _validate_rule(item.context, rules, val, data, index);
            }

            for (var i = 0; i < rules.length; i++) {
                if (!_validate_rule(item.context, rules[i], val, data, index)) {
                    result = false;
                    err_rule = rules[i];
                    break;
                }
            ...
            if (!result) {
                ...
                // function checkElForm(fi){
                //     var p = fi.parent();
                //     if(p.length > 0){
                //         p.hasClass()
                //     }
                // }
                var err_class = 'ui-item__error';
                if (!$el.parent().hasClass("el-form-item__content")) {
                    err_class = 'ui-item__error_normal';
                }
                ...
                if (showMsg) {
                    if ($fi.fitype === "textarea") {
                        var aa = "<div class='ui-item__error_container'>";
                        aa += "<div class='ui-item__error__textarea' style='width:" + $el.css("width") + "; text-align:left;'>" + err_rule.message + "</div>";
                        aa += "</div>";
                        $el.after(aa);
                    } else {
                        var aa = "<div class='ui-item__error_container'>";
                        aa += "<div class='" + err_class + "' style='width:" + $el.css("width") + "; text-align:left;'>" + err_rule.message + "</div>";
                        aa += "</div>";
                        $el.after(aa);
                    }
                }
        ...
        //单个规则的验证
        var _validate_rule = function (context, rule, val, data, index) {
            var a = {
                //不能为null和undefined
                ...
                money_no_zero: function (arg) {
                    ...
                    return $UF.validate.money_no_zero(val.trim());
                },
                //验证为非负数（包括正整数、正小数、和0）
            ...
            for (var key in rule) {
                ...
                if ($UF.isFunction(rule[key])) {
                    var v = rule[key].call(context, val, index, data);
                    if ($UF.isBoolean(v)) {
                        result = result && v;
        ...
        var _addItem = function (uid, el, rule, context, index, data, showMsg) {
            ...
            _config.validate_group[group_name].push({uid: uid, el: el, context: context, rules: rules, condition: condition, index: index, data: data, showMsg: showMsg});

            //绑定blur事件
            var $fi = _get_input(el);
            var f = function () {
                _validate_item({uid: uid, el: el, context: context, rules: rules, condition: condition, index: index, data: data, showMsg: showMsg})
            };

            _add_valid_event($fi, f);
        };
        ...
        $$.install = function (Vue, options) {
            ...
            Vue.directive('ui-rule', {
                ...
                inserted: function (el, binding, vnode) {
                    ...
                    // if ($UF.isNumber(rule.index) || rule.data) {
                    rule.showMsg = !$UF.isBoolean(rule.showMsg) || rule.showMsg;
                    // if (!$UF.validate.null(rule.index) || rule.data) {
                    if (rule.hasOwnProperty("rule")) {
                        _addItem(vnode.componentInstance._uid, el, rule.rule, vnode.context, rule.index, rule.data, rule.showMsg);
                    } else {
                        _addItem(vnode.componentInstance._uid, el, rule, vnode.context, null, null, true);
                    }
...
var $UD = (function (w, $) {
    ...
    var _getList = function (url, data) {
        ...
        if (url.lastIndexOf("?") > -1 && url.lastIndexOf("&") > -1) {
            url += "&_util_core_random=" + Math.random() + "";
        } else {
            url += "?_util_core_random=" + Math.random() + "";
        }
    ...
    $$.enums = function () {
        ...
        return {
            ...
            /**
             * 获取单个枚举的集合(满足not in filterArray)
             *
             * @param enumName 枚举名
             * @param filterArray array 要排除的枚举项
             * @returns {*}
             */
            listOneExclude: function (enumName, filterArray) {
                if (!$UF.isArray(filterArray)) {
                ...
                var find = this.listOne(enumName);
                if (find == null) {
                    return [];
                }
                var list = [];
                for (var key in find) {
                    var code = find[key].value;
                    if (!$UF.inArray(code, filterArray)) {
                        list.push(find[key]);
                    }
                }
                return list;
    ...
    $$.addRemoteResource = function (name, url, methodCall, force) {
        var _getJson = function (param) {
            if (force || !_dataCache[name]) {
                _dataCache[name] = _getList(url, param);
            }
